  ...
  ngAfterViewInit() {
    ...
    const dragged = (event, d) => {
      ...
      if (rotationMode === 'rotation') {
        const rot =
          (Math.atan2(y - d.y + adjy, x - d.x + adjx) * 180) / Math.PI;
        s.attr(
          'transform',
          `translate(${d.x},${d.y}) rotate(${(d.rot = rot)})`
        );
      } else {
        s.attr(
          'transform',
