...
@Component({
  ...
  template: `
    <div class="header">
      <form [formGroup]="form">
        <button mat-icon-button (click)="decrementWeek()">
          <mat-icon>chevron_left</mat-icon>
        </button>
        <div class="select">
          <mat-select #select formControlName="week" required>
            <mat-option *ngFor="let week of weeks$ | async" [value]="week">{{
              formatWeekText(week)
            }}</mat-option>
          </mat-select>
          <button mat-stroked-button (click)="select.open()" class="select">
            {{ formatWeekText(form.get('week').value) }}
          </button>
        </div>
        <button mat-icon-button (click)="incrementWeek()">
      ...
      <div class="timecard">
        <ng-container *ngFor="let datum of data$ | async">
          ...
          <div>
            <span *ngFor="let shift of datum.shifts">{{ shift | json }}</span>
          </div>
        </ng-container>
      </div>
    </div>
...
export class TimecardComponent implements OnInit, OnChanges {
  ...
  week$ = this.form
    .get('week')
    .valueChanges.pipe(startWith(this.form.get('week').value));
  ...
  formatWeekText(week: string) {
    ...
    return `Week of ${sun.getMonth() + 1}/${sun.getDate()} - ${
      sat.getMonth() + 1
    }/${sat.getDate()}${
      week === serializeDate(this.thisWeek) ? ' (this week)' : ''
    }`;
  }
