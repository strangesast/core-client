...
export class MapViewerComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  height: number;
  assets$ = this.http
    .get('/assets/df_building.json')
    .pipe(multicast(new BehaviorSubject(null)), refCount());
  dirty = false;
  ...
  ngAfterViewInit(): void {
    ...
    combineLatest(
      ...
      const data = Object.entries(json.objects).filter(
        (d) => d[0] !== 'cameras' && d[0] !== 'building'
      );

      g.selectAll('g.object')
        .data(data, (d) => d[0])
        .join((s) =>
          s
            .append('g')
            .classed('object', true)
            .call((ss) => {
              ss.append('path').attr('fill', 'none').attr('stroke', 'black');
        ...
        .on('click', (d) => {
          ...
          if (this.lastClicked === d[0]) {
            ...
            svg
              ...
              .call(
                zoom.transform,
                calcZoom(topojson.feature(json, d[1]), false)
              );
          }
        });
