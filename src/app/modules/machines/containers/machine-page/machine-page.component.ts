...
const query = gql`
  query ExecutionState(
    ...
    part_counts: machine_state(
      where: {
        _and: {
          machine_id: { _eq: $machineID }
          property: { _eq: "part_count" }
          timestamp: { _gte: $minDate, _lt: $maxDate }
        }
      }
      order_by: { timestamp: asc }
...
@Component({
  ...
  -->
  `,
  styleUrls: ['../../../../styles/base.scss', './machine-page.component.scss'],
})
export class MachinePageComponent implements OnInit, AfterViewInit {
  ...
  machineID$ = this.route.params.pipe(pluck('id'));
  ...
  ngAfterViewInit() {
    ...
    this.svg
      .append('g')
      .classed('x-axis', true)
      .call((this.xAxis = d3.axisBottom(this.xScale)));
  }
  ...
  draw(data) {
    ...
    this.svg
      .selectAll('g.execution')
      .data(arr)
      .join((s) =>
        s
          .append('g')
          .classed('execution', true)
          .call((ss) => ss.append('rect').attr('height', 40))
      )
    ...
    this.svg
      .selectAll('g.part_count')
      .data(part_counts)
      .join((s) =>
        s
          .append('g')
          .classed('part_count', true)
          .call((ss) => {
            ss.append('circle')
              .attr('r', 10)
              .attr('fill', 'none')
      ...
      .attr(
        'transform',
        (d: any) => `translate(${this.xScale(d.timestamp)},20)`
      );
