...
const query = gql`
  query($from: timestamptz, $to: timestamptz) {
    machine_execution_state(
      where: {
        _and: [
          ...
          {
            _or: [
              { next_timestamp: { _is_null: true } }
              { next_timestamp: { _lt: $to } }
            ]
          }
        ]
      }
    ...
    bounds: machine_execution_state(
      distinct_on: machine_id
      order_by: { machine_id: asc, timestamp: asc }
      where: {
        _and: [
          { next_timestamp: { _gt: $from } }
          { timestamp: { _lte: $from } }
        ]
      }
    ) {
...
export class MachineActivityGraphComponent extends BaseGraphComponent
  ...
  data$ = combineLatest(
    ...
    this.variables$.pipe(
      ...
      map(([{ machine_execution_state, bounds }, range]: any) => {
        const arr = machine_execution_state.map(
          ({ machine_id, next_timestamp, timestamp, offset, value }) => ({
            timestamp:
              timestamp &&
              new Date(
            ...
            next_timestamp: next_timestamp
              ? new Date(
                  next_timestamp.endsWith('+00:00')
                    ? next_timestamp.slice(0, -6) + 'Z'
                    : next_timestamp
                )
              : new Date(),
            offset,
        ...
        const boundsByKey = bounds.reduce(
          (acc, val) => ({ ...acc, [val.machine_id]: val.value }),
          {}
        );
        for (const [key, each] of grouped) {
          each.unshift({
            ...each[0],
            timestamp: range[0],
            next_timestamp: each[0].timestamp,
            value: boundsByKey[key],
          });
        }
        return grouped;
      })
  ...
  ngAfterViewInit() {
    ...
    this.data$
      .pipe(withLatestFrom(this.range$), takeUntil(this.destroyed$))
      .subscribe(([data, range]: [any[], any]) => {
        xScale.domain(range);
        gx.attr(
          'transform',
          `translate(0,${data.length * 70 + margin.top})`
        ).call(xAxis);
        ...
        g.selectAll('g')
          ...
          .attr(
            'width',
            (d: any) => xScale(d.next_timestamp) - xScale(d.timestamp)
          )
          .attr('fill', (d: any) => {
            switch (d.value) {
              case 'ACTIVE':
