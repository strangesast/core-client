...
export class MachineLiveGraphComponent
  ...
  ngAfterViewInit() {
    ...
    this.data$
      .pipe(withLatestFrom(this.minDate$))
      .subscribe(([data, minDate]: any[]) => {
        const keys = [
          'Xload',
          'Zload',
          'S1load',
        ...
        this.svg
          .selectAll('g')
          .data(
            keys.map((key) => [key, grouped.get(key) || []]),
            (d) => d[0]
          )
          .join(
            (s) =>
              s.append('g').call((ss) => {
                ss.append('path').attr('stroke', 'black').attr('fill', 'none');
                ss.append('text')
          ...
          .each(function (d: any) {
            ...
            const line = d3
              .line()
              .x((dd: any) => xScale(dd.timestamp))
              .y((dd: any) => yScale(dd.value));
