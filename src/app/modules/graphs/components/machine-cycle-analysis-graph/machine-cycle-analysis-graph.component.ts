...
export class MachineCycleAnalysisGraphComponent
  ...
  ngAfterViewInit() {
    ...
    this.data$.subscribe((data: any[]) => {
      const groups = (group as any)(
        data.map(({ machine_id, offset, property, r, timestamp, value }) => ({
          machine_id,
          property,
          value,
      ...
      const colors = (Object as any).fromEntries(
        keys.map((k, i) => [k, d3.schemeSet1[i]])
      );
      ...
      this.svg.style(
        'height',
        `${eachHeight * 20 + margin.top + legendHeight + margin.bottom}px`
      );
      ...
      g.selectAll('g.machine')
        .data(Array.from(groups), (d) => d[0])
        .join((s) =>
          s
            .append('g')
        ...
        .each(function (d: any) {
          ...
          const extent = (d3.extent as any)(
            Array.from(d[1]).reduce(
              (acc: any, dd: any) => acc.concat(dd[1]),
              []
            ),
            (dd) => dd.timestamp
          );
          const xScale = d3
            .scaleTime()
            .domain(extent)
            .range([padding / 2, eachWidth - padding / 2]);

          s.selectAll('path')
            .data(
              Array.from(d[1]).filter((dd) => dd[0].endsWith('load')),
              (dd) => dd[0]
            )
            .join((ss) =>
              ss
            ...
            .each(function (dd: any) {
              ...
              const arr = dd[1].map((ddd) => ({
                ...ddd,
                value: parseValue(ddd.property, ddd.value),
              }));
              const yExtent = d3.extent(arr, (ddd: any) => ddd.value) as any;
              const yScale = d3.scaleLinear().domain(yExtent).range([80, 0]);
              const line = d3
                .line()
                .x((ddd: any) => xScale(ddd.timestamp))
                .y((ddd: any) => yScale(ddd.value));
