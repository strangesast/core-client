  ...
  template: `
    ...
    <header>
      <ng-container *ngIf="active$ | async as active">
        ...
        <p>
          As of
          <span *ngIf="isToday(active.asof); else old">{{
            active.asof | date: 'mediumTime'
          }}</span
          ><ng-template #old>{{ active.asof | date: 'medium' }}</ng-template
          >, 34 Total Hours Today, <a [routerLink]="['/graphs']">Graphs</a>
        </p>
      </ng-container>
    </header>
    <div class="controls">
...
export class TimeclockPageComponent implements OnInit {
      ...
      query: gql`
        query MyQuery($timestamp: timestamp) {
          timeclock_shifts(
            ...
            where: {
              date_stop: { _is_null: true }
              date_start: { _gt: $timestamp }
            }
          ) {
            date_start
            employee {
  ...
  constructor(
    ...
    this.date$
      .pipe(first())
      .subscribe((date) => this.form.patchValue({ date }));
    ...
    this.data$ = this.range$.pipe(
      ...
      switchMap(([range, includeActive]) =>
        this.apollo.query({
          query: gql`
            query TimeclockShifts(
              $_gte: timestamp
              $_lte: timestamp
              ...
              timeclock_shifts(
                where: {
                  _and: [
                    { date_start: { _gte: $_gte } }
                    {
                      _or: [
                        { date_stop: { _is_null: $includeActive } }
                        { date_stop: { _lte: $_lte } }
                      ]
                    }
                  ]
          ...
          `,
          variables: {
            _gte: range[0].toISOString(),
            _lte: range[1].toISOString(),
            includeActive,
          },
        })
      ),
  ...
  ngOnInit(): void {
    ...
    this.data$.subscribe((data) => {
      data = Array.from(
        ...
        .map(([_, shifts]) =>
          shifts.sort((a, b) => (a.date_start < b.date_start ? -1 : 1))
        )
        ...
        .map((shifts) => {
          ...
          if (shifts[shifts.length - 1].duration == null) {
            ...
            duration = of(
              shifts.reduce((acc, shift) => acc + shift.duration, 0)
            );
          }
          return {
