...
const periodSummaryQuery = gql`
  query MyQuery($minDate: timestamp, $maxDate: timestamp, $employeeId: Int) {
    timeclock_shifts(
      where: {
        _and: {
          date_start: { _gte: $minDate }
          date_stop: { _lt: $maxDate }
      ...
      <div class="table-container">
        ...
        >
          ...
          <ng-container matColumnDef="parts">
            ...
            <td mat-cell *matCellDef="let row">
              <div
                [ngStyle]="{ 'min-width.px': totalWidth, overflow: 'hidden' }"
                class="blocks"
              >
                <span
                  *ngFor="let shift of row.shifts; ngForTrackBy: trackBy"
                  matTooltip="{{ shift.date_start | date: 'short' }} - {{
                    shift.date_stop | date: 'shortTime'
                  }}"
          ...
          <ng-container matColumnDef="total" stickyEnd>
            ...
            <td mat-cell *matCellDef="let row">
              {{
                getDuration(row.duration, row.date_start)
                  | async
                  | duration: 'h:mm:ss'
              }}
            </td>
          ...
          <ng-container matColumnDef="expandedDetail">
            ...
            >
              ...
              >
                <ng-container
                  *ngIf="
                    expanded === row && expandedData$ | async as data;
                    else loading
                  "
                >
                  <div class="days">
                    <div class="day" *ngFor="let each of data.days">
                      <span>{{
                        each.value > 0 ? each.value.toFixed(1) : ''
                      }}</span>
                      <span
                        class="pill vert"
                        [matTooltip]="each.value.toFixed(2)"
                        [ngStyle]="{
                          height: each.frac * 40 + 'px',
                          'background-color': row.employee.color
                        }"
                      >
                      </span>
                      <div class="label">
...
export class SlidyTableComponent
  ...
  ngOnInit(): void {
    this.date$
      .pipe(
        switchMap(
          (date) =>
            this.apollo.watchQuery({ query, variables: { date } }).valueChanges
        ),
        pluck('data', 'shifts'),
        map((arr: any[]) => {
          let minDate;
  ...
  getDays(ofDate, employeeId) {
    ...
    minDate.setDate(
      minDate.getDate() - minDate.getDay() - ((weekNo + 1) % 2) * 7
    );
    ...
    return this.apollo.query({ query: periodSummaryQuery, variables }).pipe(
      ...
      map((arr: any[]) =>
        arr.map(({ date, date_start, date_stop }) => ({
          date,
      ...
      map((arr: any[]) => {
        ...
        for (const [key, value] of group(arr, (d) => d.date)) {
          ...
          const val =
            value.reduce(
              (acc, shift) => acc + (+shift.date_stop - +shift.date_start),
              0
            ) / 3.6e6;
          total += val;
