      ...
      <div class="labels">
        <div
          *ngFor="let label of labels$ | async; index as i"
          class="label"
          [ngStyle]="{ transform: getTransform(i, 0) }"
        >
...
export class TimeclockStaggeredComponent
    ...
    resize$
      .pipe(
        ...
        switchMap(({ width, height }) =>
          scrolled$.pipe(
            map(() => ({
              offset: this.scroller.measureScrollOffset('left'),
              width,
              height,
            }))
          )
  ...
  getHostElementSize(): { width: number; height: number } {
    return this.scroller.getElementRef().nativeElement.getBoundingClientRect();
  }
