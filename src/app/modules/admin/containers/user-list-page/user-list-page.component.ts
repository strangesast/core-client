  ...
  template: `
    ...
    <table mat-table [dataSource]="dataSource">
      ...
      <ng-container matColumnDef="employee">
        ...
        <td mat-cell *matCellDef="let row">
          <a *ngIf="row.employee as empl" [routerLink]="['/people', empl.id]"
            >{{ empl.first_name }} {{ empl.last_name }}</a
          >
        </td>
      </ng-container>
      <!-- Symbol Column -->
...
export class UserListPageComponent implements OnInit, OnDestroy {
  ...
  checkboxLabel(row?: UserRecord): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} user ${
      row.id
    }`;
  }
  ...
  ngOnInit(): void {
    this.getEmployees()
      .pipe(takeUntil(this.destroyed$), pluck('data', 'users'))
      .subscribe((data: any) => {
        data = data.map((datum) => {
